...
пока все обновляют страницу руками — этот уже отправил уведомление. **не спит, не устаёт, не пропускает.** написан на go — компилируется за секунду, **весит 15mb, летит быстрее чем ты успел подумать.**
